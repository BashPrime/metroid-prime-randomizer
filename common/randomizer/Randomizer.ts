...
export class Randomizer {
  ...
  randomize(): void {
    ...
    // First, fill vanilla/items not being shuffled in the seed
    ...
    // These "junk" items do not affect progression, and the progression items are set, so these items have no restrictions.
  ...
  fillUnshuffledItems(settings: any): void {
    ...
    // Charge Beam
    if (settings['shuffleCharge']) {
      locations.get('Watery Hall (Scan Puzzle)').setItem(Item.get(PrimeItem.MORPH_BALL_BOMB));
    }

    // Super Missile
    if (settings['shuffleSupers']) {
      locations.get('Observatory').setItem(Item.get(PrimeItem.MORPH_BALL_BOMB));
    }
