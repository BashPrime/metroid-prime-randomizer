...
export class ItemCollection extends Collection {
  ...
  public hasLateMagmoorItemReqs(settings: any): boolean {
    ...
    if (this.hasMinesReqsTallonSouth(settings) && ((settings.standableTerrain && settings.spiderlessPPC) || this.has(PrimeItem.SPIDER_BALL)) && this.canLayPowerBombs()) {
      return true;
    }
  ...
  public hasMinesReqsTallonSouth(settings): boolean {
    return (settings.barsSkip && this.canLayBombs() && this.hasReflectingPoolReqs(settings) && this.has(PrimeItem.WAVE_BEAM) && this.has(PrimeItem.ICE_BEAM)) // bars skip
