  ...
  public init(settings: any): void {
    ...
    this.locations.get(PrimeLocation.PLASMA_PROCESSING).canFillItem = function (item: Item, items: ItemCollection): boolean {
      ...
      if (settings.workstationToPlasmaProcessing) {
        ...
        && (items.has(PrimeItem.ICE_BEAM) || (!(settings.noVanillaBeams || settings.hideItemModels) || items.has(PrimeItem.PLASMA_BEAM)));
      }
      ...
      && (!(settings.noVanillaBeams || settings.hideItemModels) || items.has(PrimeItem.PLASMA_BEAM)) // require plasma if no vanilla beams is checked
